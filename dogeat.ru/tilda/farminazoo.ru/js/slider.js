(function () {
  ...
  const TOP_TABS_SPEED = 450;
  const PANEL_SWITCH_CLASS = "species-panel--switching";
  const PANEL_SWITCH_CLEAR_MS = 900;
  if (typeof window.Swiper !== "function") return;

  const topTabsEl = document.querySelector(".top-tabs");
  const topTabsSwiper = topTabsEl
    ? new Swiper(topTabsEl, {
        slidesPerView: "auto",
        freeMode: true,
        allowTouchMove: true,
        speed: TOP_TABS_SPEED,
        slideToClickedSlide: true,
      })
    : null;
  const topTabsTrack = topTabsEl?.querySelector(".swiper-wrapper") || null;
  ...
  let panelSwitchTimeout = null;

  function updateTopTabsIndicator(index) {
    if (!topTabsTrack) return;

    const activeTab = spTabs[index];
    if (!activeTab) return;

    const trackRect = topTabsTrack.getBoundingClientRect();
    const tabRect = activeTab.getBoundingClientRect();
    const tabLeft = tabRect.left - trackRect.left;

    topTabsTrack.style.setProperty("--fa-tab-left", `${tabLeft}px`);
    topTabsTrack.style.setProperty("--fa-tab-width", `${tabRect.width}px`);
  }

  function animateActivePanel(panel) {
    if (!panel || prefersReducedMotion) return;

    const midTabSlides = Array.from(panel.querySelectorAll(".mid-tabs .swiper-slide"));
    midTabSlides.forEach((slide, idx) => {
      slide.style.setProperty("--fa-mid-index", `${idx}`);
    });

    const cards = Array.from(panel.querySelectorAll(".card"));
    cards.forEach((card, idx) => {
      card.style.setProperty("--fa-card-index", `${idx}`);
    });

    panel.classList.remove(PANEL_SWITCH_CLASS);
    void panel.offsetWidth;
    panel.classList.add(PANEL_SWITCH_CLASS);

    if (panelSwitchTimeout) {
      window.clearTimeout(panelSwitchTimeout);
    }

    panelSwitchTimeout = window.setTimeout(() => {
      panel.classList.remove(PANEL_SWITCH_CLASS);
      panelSwitchTimeout = null;
    }, PANEL_SWITCH_CLEAR_MS);
  }

  function setSpeciesActive(
    index,
    { focus = false, reset = true, animate = true } = {},
  ) {
    const changed = index !== activeSpeciesIndex;
    ...
    spPanels.forEach((panel, i) => {
      ...
      if (!panel) return;
      panel.hidden = !show;
      panel.classList.toggle("species-panel--active", show);
    });

    updateTopTabsIndicator(index);

    if (changed && animate) {
      animateActivePanel(spPanels[index]);
    }

    if (topTabsSwiper && typeof topTabsSwiper.slideTo === "function") {
      topTabsSwiper.slideTo(index, TOP_TABS_SPEED);
    }
  ...
  setSpeciesActive(0, { animate: false });

  window.addEventListener("resize", () => {
    updateTopTabsIndicator(activeSpeciesIndex);
  });
